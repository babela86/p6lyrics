...
import dei.uc.pt.ar.adomain.entities.SimpleUser;
...
public class SimpleUserFacade implements Serializable, ISimpleUserFacade {
	...
	public SimpleUser find(long pk) {
		// findById
		TypedQuery<SimpleUser> query = entityManager.createNamedQuery(
				"SimpleUser.findById", SimpleUser.class).setParameter("id", pk);
	...
	public Collection<SimpleUser> findAll() {
		// THE NOT SO GOOD WAY
		// Query query =
		// entityManager.createQuery("SELECT user FROM SimpleUser user");
		// SimpleUser.findAll
		...
		// THE GOOD WAY
		TypedQuery<SimpleUser> query = entityManager.createNamedQuery(
				"SimpleUser.findAll", SimpleUser.class);
